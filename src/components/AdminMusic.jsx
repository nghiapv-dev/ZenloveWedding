      ...
      <section className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-2xl items-center">
        <div className="w-full rounded-3xl border border-rose-100 bg-white p-6 shadow-[0_18px_50px_rgba(229,65,83,0.12)]">
          ...
          <h1 className="mt-2 text-2xl font-extrabold">
            Kho nhạc cloud chưa được kết nối
          </h1>
          <p className="mt-3 text-sm leading-6 text-slate-500">
            Thêm các biến môi trường dưới đây vào Vercel và file .env.local trên
            máy để bật upload nhạc lên cloud.
          </p>
    ...
    <main className="min-h-screen bg-[#fff7f8] px-4 py-8 text-slate-950">
      <section className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-md items-center">
        ...
        >
          ...
          <p className="mt-3 text-sm leading-6 text-slate-500">
            Đăng nhập tài khoản admin Supabase để upload, nghe thử và tải nhạc
            cưới trên mọi thiết bị.
          </p>
          <input
          ...
          <button
            className="mt-5 h-12 w-full rounded-xl bg-[#E54153] text-sm font-extrabold text-white shadow-lg shadow-rose-200 transition hover:bg-[#c93345] disabled:opacity-60"
            disabled={isLoading}
          >
...
function AdminMusic() {
  ...
  const handleUpload = async (event) => {
    ...
    const existingNames = new Set(
      songs.map((song) => song.file_name.toLowerCase()),
    );
    ...
    selectedFiles.forEach((file) => {
      ...
      if (
        ...
        uploadNames.has(normalizedName)
      ) {
    ...
    try {
      ...
      event.target.value = "";
      const skippedText = duplicateFiles.length
        ? ` Bỏ qua ${duplicateFiles.length} file trùng tên.`
        : "";
  ...
  const handleDelete = async (song) => {
    const confirmed = window.confirm(
      `Xóa bài "${song.title}" khỏi kho nhạc cloud?`,
    );
    if (!confirmed) return;
  ...
  const saveTitle = async (song) => {
    ...
    try {
      ...
      setSongs((current) =>
        current.map((item) =>
          item.id === song.id ? { ...item, title } : item,
        ),
      );
      ...
      <section className="mx-auto max-w-7xl">
        ...
        <div className="mb-5 flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-rose-100 bg-white p-4 shadow-[0_14px_36px_rgba(229,65,83,0.08)] sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-[#E54153]">
                Zenlove Wedding / Admin
              </p>
              <h1 className="mt-1 text-xl font-extrabold sm:text-2xl">
                Quản lý kho nhạc cưới
              </h1>
          ...
          <div className="flex gap-2">
            <button
              className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#E54153] px-4 text-sm font-bold text-white"
              onClick={() => setShowAddForm((current) => !current)}
              type="button"
            >
              <Plus size={16} />
              {showAddForm ? "Đóng form" : "Thêm nhạc"}
            </button>
            <button
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-rose-100 bg-white px-4 py-3">
            <p className="text-xs font-bold text-slate-500">Tổng bài nhạc</p>
            <p className="mt-1 text-2xl font-extrabold">{songs.length}</p>
          </div>
          <div className="rounded-2xl border border-rose-100 bg-white px-4 py-3">
          ...
          <div className="rounded-2xl border border-rose-100 bg-[#fff1f3] px-4 py-3">
            <p className="text-xs font-bold text-[#b92d3e]">Trạng thái kho</p>
            <p className="mt-1 text-sm font-extrabold text-[#E54153]">
              Cloud đã kết nối
            </p>
          </div>
        </div>
        <div
          className={`grid gap-5 ${showAddForm ? "xl:grid-cols-[340px_minmax(0,1fr)]" : ""}`}
        >
          <aside
            className={`rounded-3xl border border-rose-100 bg-white p-4 shadow-[0_14px_36px_rgba(229,65,83,0.08)] sm:p-5 ${showAddForm ? "" : "hidden"}`}
          >
            <p className="text-[11px] font-extrabold uppercase tracking-[0.14em] text-[#E54153]">
              Thư viện âm thanh
            </p>
            <h2 className="mt-1 text-lg font-extrabold">Thêm nhạc mới</h2>
            ...
            <label className="mt-4 flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-rose-200 bg-[#fffafa] px-4 text-center transition hover:border-rose-400 hover:bg-rose-50">
              ...
              <span className="text-sm font-extrabold text-slate-900">
                Bấm để upload MP3 lên cloud
              </span>
              <span className="mt-1 text-xs text-slate-500">
                Có thể chọn nhiều bài cùng lúc
              </span>
              <input
                accept="audio/*"
                className="hidden"
          ...
          <section className="rounded-3xl border border-rose-100 bg-white p-4 shadow-[0_14px_36px_rgba(229,65,83,0.08)] sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row">
              <label className="relative flex-1">
                <Search
                  className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-rose-500"
                  size={18}
                />
                <input
            ...
            <div className="mt-5 flex items-center justify-between gap-3">
              ...
              <span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-extrabold text-rose-500">
                {filteredSongs.length} bài • Trang {safeCurrentPage}/
                {totalPages}
              </span>
            </div>
            ...
            <div className="mt-4 grid gap-3">
              {isLoading ? (
                <div className="rounded-2xl bg-rose-50 p-4 text-sm font-bold text-slate-500">
                  Đang xử lý...
                </div>
              ) : null}
              {!isLoading && !filteredSongs.length ? (
                <div className="rounded-2xl border border-dashed border-rose-100 bg-rose-50/50 p-6 text-center">
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Upload MP3 ở khung bên trái để bắt đầu.
                  </p>
                </div>
              ) : null}
                    ...
                    <div className="min-w-0 flex-1">
                      {editingSongId === song.id ? (
                        <div className="flex flex-col gap-2 sm:flex-row">
                          ...
                          <div className="flex gap-2">
                            <button
                              className="h-10 rounded-xl bg-rose-500 px-3 text-sm font-extrabold text-white disabled:opacity-50"
                              disabled={isLoading}
                              onClick={() => saveTitle(song)}
                              type="button"
                            >
                              Lưu
                            </button>
                            <button
                              className="h-10 rounded-xl border border-rose-100 px-3 text-sm font-extrabold text-slate-600"
              ...
              {filteredSongs.length > songsPerPage ? (
                <div className="flex flex-col gap-3 rounded-xl border border-rose-100 bg-[#fffafa] p-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-center text-sm font-bold text-slate-500 sm:text-left">
                    ...
                    trong {filteredSongs.length} bài
                  </p>
